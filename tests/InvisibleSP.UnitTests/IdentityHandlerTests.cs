...
public sealed class IdentityHandlerTests
{
    ...
    public async Task All_identity_handlers_should_delegate_to_identity_service()
    {
        ...
        (await new LoginCommandHandler(service).Handle(new ReceiveContext<LoginCommand>(new("a@b.com", "Password1!")), CancellationToken.None)).Should().NotBeNull();
        (await new RefreshTokenCommandHandler(service).Handle(new ReceiveContext<RefreshTokenCommand>(new("refresh")), CancellationToken.None)).Should().NotBeNull();
        (await new RevokeTokenCommandHandler(service).Handle(new ReceiveContext<RevokeTokenCommand>(new("access")), CancellationToken.None)).Should().BeTrue();
        (await new ConfirmEmailCommandHandler(service).Handle(new ReceiveContext<ConfirmEmailCommand>(new("id", "code")), CancellationToken.None)).Should().BeTrue();
        ...
        (await new GetIdentityInfoQueryHandler(service).Handle(new ReceiveContext<GetIdentityInfoQuery>(new("id")), CancellationToken.None)).Should().NotBeNull();
        ...
        (await new ConfigureTwoFactorCommandHandler(service).Handle(new ReceiveContext<ConfigureTwoFactorCommand>(new("id", null, null, false, false, false)), CancellationToken.None)).Should().NotBeNull();
