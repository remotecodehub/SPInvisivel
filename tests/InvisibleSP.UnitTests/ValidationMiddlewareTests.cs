...
public sealed class ValidationMiddlewareTests
{
    ...
    public async Task Fluent_message_validator_should_execute_registered_validator()
    {
        ...
        var valid = () => validator.ValidateAsync(new RegisterCommand("user@example.com", "Password1!"), CancellationToken.None);
        await valid();

        var invalid = () => validator.ValidateAsync(new RegisterCommand("invalid", "short"), CancellationToken.None);
